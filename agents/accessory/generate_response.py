...
class SystemPromptConfig(SystemPromptConfigBase):
    role: str = "You are professional sales consultant staff for an accessory store."

    task: str = (
        "Your task is to assist users in selecting suitable accessories and providing guidance on purchasing procedures."
    )
    skills: list[str] = [
    ...
    base_knowledge: list[str] = [
        (
            "Information about your accessory store:\n"
            "   - Name: FPTShop\n"
    ...
    def accessory_knowledge_to_string(self) -> str:
        if len(self.accessory_knowledge) == 0:
            return ""

        if len(self.accessory_knowledge) == 1:
            return self.accessory_knowledge[0]

        text = "\n".join(
            [
                f"<ACCESSORY {i + 1}>\n{knowledge}\n</ACCESSORY {i + 1}>"
                for i, knowledge in enumerate(self.accessory_knowledge)
            ]
        )
...
{self.skills_to_string()}""",
        ...
        accessory_knowledge_message: ChatCompletionMessageParam | None = (
            {
                "role": "system",
                "content": f"""## ACCESSORY KNOWLEDGE:\n{self.accessory_knowledge_to_string()}""",
            }
            if self.accessory_knowledge
            else None
        )
...
class Agent(AgentBase):
    ...
    def run(
        ...
        **kwargs,
        ...
        self.system_prompt_config.accessory_knowledge = accessory_knowledge
