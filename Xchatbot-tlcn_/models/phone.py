...
class Phone(Base):
    ...
    data: Mapped[dict] = mapped_column(JSONB, nullable=False, default={}) #binary json
...
class PhoneModel(BaseModel):
    ...
    def _get_original_price(self) -> int:
        return self.data.get("original_price", 0)
    
    def _get_current_price(self) -> int:
        return self.data.get("current_price", 0)
    ...
    def _get_key_selling_points_text(
        ...
        selling_points_texts = []
        for point in self.key_selling_points:
            point_text = f"{prefix}{point['title']}"
            description = point.get("description","")
            if len(description) > 0 :
                point_text += f" {description}"
            selling_points_texts.append(point_text)

        return(
            separator.join(selling_points_texts) if len(selling_points_texts) > 0 else None
        ) 
    ...
    def _get_sku_variants_text(
        ...
        for sku in self.skus:
            ...
            for variant in sku.get("variants", []):
                variant_text = f"{variant['displayname']} {variant['propertyName']}"
    ...
    def is_on_sale(self) -> bool:
        return self.get_củrent_price() < self.get_original_price()
    ...
    def to_text(
        ...
        if include_key_selling_points:
            ...
            result += (
                ...
                if key_selling_points_text else ""
            )
        
        if include_promotion:
            promotion_text = self._get_promotion_text()
            result += f"- Promotion: \n{promotion_text}\n" if promotion_text else ""

        if include_sku_variants:
            sku_variant_text = self._get_sku_variants_text()
            result += f" Variants: {sku_variant_text}\n" if sku_variant_text else ""
