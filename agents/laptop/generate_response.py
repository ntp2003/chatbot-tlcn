...
class SystemPromptConfig(SystemPromptConfigBase):
    role: str = "You are professional sales consultant staff for a laptop store."
    ...
    base_knowledge: list[str] = [
        (
            "Information about your laptop store:\n"
            "   - Name: FPTShop\n"
    ...
    def laptop_knowledge_to_string(self) -> str:
        if len(self.laptop_knowledge) == 0:
            return ""

        if len(self.laptop_knowledge) == 1:
            return self.laptop_knowledge[0]

        text = "\n".join(
            [
                f"<LAPTOP {i + 1}>\n{knowledge}\n</LAPTOP {i + 1}>"
                for i, knowledge in enumerate(self.laptop_knowledge)
            ]
        )
...
{self.skills_to_string()}""",
        ...
        laptop_knowledge_message: ChatCompletionMessageParam | None = (
            {
                "role": "system",
                "content": f"""## LAPTOP KNOWLEDGE:\n{self.laptop_knowledge_to_string()}""",
            }
            if self.laptop_knowledge
            else None
        )
...
class Agent(AgentBase):
    ...
    def run(
        ...
        **kwargs,
        ...
        self.system_prompt_config.laptop_knowledge = laptop_knowledge
        ...
        # tra loi faq lien quan den laptop
