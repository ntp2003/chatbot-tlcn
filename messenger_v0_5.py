...
async def process_message(sender_id: str, message_text: str):
    ...
    try:
        # Create persistent thread ID
        persistent_thread_id = uuid5(uuid.NAMESPACE_DNS, sender_id)
        
        # Get user info from Facebook including gender
        url = f"https://graph.facebook.com/{sender_id}?fields=name,gender&access_token={PAGE_ACCESS_TOKEN}"
        async with httpx.AsyncClient() as client:
            response = await client.get(url)
            fb_user_info = response.json()
            user_name = fb_user_info.get('name')
            user_gender = fb_user_info.get('gender', 'unknown')  # Get gender, default to 'unknown'
            logger.info(f"User info retrieved: {user_name}, Gender: {user_gender}")
            logger.info(f"FB_id: {sender_id}")

        # Check/Create user in database
        user = get_user_by_fb_id(sender_id)
        if not user:
            logger.info(f"User with fb_id {sender_id} not found. Creating new user.")
            user = create_user(CreateUserModel(
                user_name=user_name,
                fb_id=sender_id,
                gender=user_gender  # Add gender to user creation
            ))
        ...
        if sender_id not in conversation_history:
            conversation_history[sender_id] = ConversationState(
                ...
                gender=user_gender,  # Add gender to conversation state
        ...
        # Add user message to history with gender context
        conversation.messages.append({
            "role": "user",
            "content": message_text,
            "gender": user_gender  # Add gender to message context
        })
        ...
        formatted_messages = [
            {
                "role": msg["role"],
                "content": msg["content"],
                "gender": msg.get("gender", "unknown")  # Include gender in formatted messages
            }
            for msg in conversation.messages
        ]

        # Generate chatbot response with gender context
...
def cleanup_old_conversations(): # xóa conversation cũ hơn 24h
